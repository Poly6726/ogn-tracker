...
esp_err_t WIFI_StartAP(const char *SSID, const char *Pass, int Channel, int MaxConnections)
{ esp_err_t Err;
...
{ uint8_t Len=0;
  ...
  Out[Len++]=' ';
  Len+=Format_UnsDec(Out+Len, (uint32_t)AP->primary, 2);
  Len+=Format_String(Out+Len, "ch/");
  Len+=Format_SignDec(Out+Len, (int32_t)AP->rssi);
