...
{ char Line[64]; int Len;
  ...
  { Len =Format_String(Line, "<option value=\"");
    Len+=Format_UnsDec(Line+Len, (uint32_t)Idx);
    Len+=Format_String(Line+Len, "\"");
...
{ char Line[16]; int Len;
  ...
  Len=Format_UnsDec(Line, (uint32_t)Parameters.NavRate);
  httpd_resp_send_chunk(Req, Line, Len);
  httpd_resp_sendstr_chunk(Req, "\">");
  End_Control_Row(Req);
  ...
  Len=Format_UnsDec(Line, (uint32_t)Parameters.NavMode);
  httpd_resp_send_chunk(Req, Line, Len);
  httpd_resp_sendstr_chunk(Req, "\">");
  End_Control_Row(Req);
  ...
  Len=Format_UnsDec(Line, (uint32_t)Parameters.PPSdelay);
  httpd_resp_send_chunk(Req, Line, Len);
  httpd_resp_sendstr_chunk(Req, "\">");
  End_Control_Row(Req);
...
*/
  ...
  Len=Format_UnsDec(Line, (uint32_t)Parameters.PageMask);
  httpd_resp_send_chunk(Req, Line, Len);
  httpd_resp_sendstr_chunk(Req, "\">");
...
{ char Line[16]; int Len;
  ...
  Len=Format_UnsDec(Line, (uint32_t)Parameters.APport);
  httpd_resp_send_chunk(Req, Line, Len);
  httpd_resp_sendstr_chunk(Req, "\">");
  End_Control_Row(Req);
  ...
  Len=Format_UnsDec(Line, (uint32_t)((10*Parameters.APtxPwr+2)>>2), 2, 1);
  httpd_resp_send_chunk(Req, Line, Len);
  httpd_resp_sendstr_chunk(Req, "\">");
  End_Control_Row(Req);
...
{ char Line[128]; int Len;
  ...
  Len+=Format_UnsDec(Line+Len, (uint32_t)GPS->Year+2000 , 4); Line[Len++]='.';
  Len+=Format_UnsDec(Line+Len, (uint32_t)GPS->Month, 2); Line[Len++]='.';
  Len+=Format_UnsDec(Line+Len, (uint32_t)GPS->Day  , 2);
  Len+=Format_String(Line+Len, "</td></tr>\n");
  httpd_resp_send_chunk(Req, Line, Len);

  Len =Format_String(Line, "<tr><td>Time</td><td align=\"right\">");
  Len+=Format_UnsDec(Line+Len, (uint32_t)GPS->Hour , 2); Line[Len++]=':';
  Len+=Format_UnsDec(Line+Len, (uint32_t)GPS->Min  , 2); Line[Len++]=':';
  Len+=Format_UnsDec(Line+Len, (uint32_t)GPS->Sec  , 2); Line[Len++]='.';
  Len+=Format_UnsDec(Line+Len, (uint32_t)GPS->mSec, 3);
  Len+=Format_String(Line+Len, "</td></tr>\n");
  httpd_resp_send_chunk(Req, Line, Len);
  ...
  Len+=Format_UnsDec(Line+Len, ((uint32_t)10*GPS_SatSNR+2)/4, 2, 1);
  ...
  { Len =Format_String(Line, "<tr><td>Pressure Alt.</td><td align=\"right\">");
    ...
    Len+=Format_SignDec(Line+Len, (int32_t)((GPS->Pressure+2)/4), 3, 2);
  ...
  Len+=Format_UnsDec(Line+Len, (uint32_t)GPS->Speed, 2, 1);
  Len+=Format_String(Line+Len, " m/s</td></tr>\n");
  httpd_resp_send_chunk(Req, Line, Len);

  Len =Format_String(Line, "<tr><td>Hor. track</td><td align=\"right\">");
  Len+=Format_UnsDec(Line+Len, (uint32_t)GPS->Heading, 4, 1);
  Len+=Format_String(Line+Len, "&deg;</td></tr>\n");
  httpd_resp_send_chunk(Req, Line, Len);
...
{ char Line[128]; int Len;
  ...
  { const LookOut_Target *Tgt = Look.Target+Idx; if(!Tgt->Alloc) continue;
    ...
    Len+=Format_UnsDec(Line+Len, (uint32_t)(Tgt->TimeMargin>>1), 2);
    Len+=Format_String(Line+Len, "s</td><td>");
    Len+=Format_UnsDec(Line+Len, (uint32_t)(((Tgt->HorDist>>1)+5)/10), 2, 2);
...
{ char Line[128]; int Len;
  ...
  Len+=Format_SignDec(Line+Len, (int32_t)Parameters.TxPower);
...
/*
  ...
  Len =Format_String(Line, "<tr><td>Temperature</td><td align=\"right\">");
  Len+=Format_SignDec(Line+Len, (int32_t)TRX.chipTemp);
  ...
  Len+=Format_UnsDec(Line+Len, (uint32_t)(RF_FreqPlan.getCenterFreq()/100000), 3, 1);
...
#endif
  Len+=Format_UnsDec(Line+Len, (uint32_t)Cap);
  Len+=Format_String(Line+Len, " %</td></tr>\n");
  httpd_resp_send_chunk(Req, Line, Len);
/*
#ifdef WITH_BQ
...
/*
  ...
  Len+=Format_UnsDec(Line+Len, (uint32_t)Vbus, 4, 3);
  Len+=Format_String(Line+Len, " V</td></tr>\n");
  httpd_resp_send_chunk(Req, Line, Len);

  Len =Format_String(Line, "<tr><td>USB curr.</td><td align=\"right\">");
  Len+=Format_UnsDec(Line+Len, (uint32_t)VbusCurr, 4, 3);
...
{ struct tm *TM = gmtime(&Time);
  int Len=Format_UnsDec(Out, (uint32_t)1900+TM->tm_year, 4);
  Out[Len++]='.';
  Len+=Format_UnsDec(Out+Len, (uint32_t)1+TM->tm_mon, 2);
  Out[Len++]='.';
  Len+=Format_UnsDec(Out+Len, (uint32_t)TM->tm_mday, 2);
  Out[Len++]=' ';
  Len+=Format_UnsDec(Out+Len, (uint32_t)TM->tm_hour, 2);
  Out[Len++]=':';
  Len+=Format_UnsDec(Out+Len, (uint32_t)TM->tm_min, 2);
  Out[Len++]=':';
  Len+=Format_UnsDec(Out+Len, (uint32_t)TM->tm_sec, 2);
  return Len; }
...
{ int Len=Parameters.getAprsCall(Name);
  Name[Len++]='_';
  struct tm *TM = gmtime(&Time);
  Len+=Format_UnsDec(Name+Len, (uint32_t)1900+TM->tm_year, 4);
  Name[Len++]='.';
  Len+=Format_UnsDec(Name+Len, (uint32_t)1+TM->tm_mon, 2);
  Name[Len++]='.';
  Len+=Format_UnsDec(Name+Len, (uint32_t)TM->tm_mday, 2);
  Name[Len++]='_';
  Len+=Format_UnsDec(Name+Len, (uint32_t)TM->tm_hour, 2);
  Len+=Format_UnsDec(Name+Len, (uint32_t)TM->tm_min, 2);
  Len+=Format_UnsDec(Name+Len, (uint32_t)TM->tm_sec, 2);
...
{ char ContDisp[64];
  ...
  Len+=Format_UnsDec(Line+Len, (uint32_t)Time.Month, 2);
  Len+=Format_UnsDec(Line+Len, (uint32_t)Time.Year , 2);
  Line[Len++]='\n';
